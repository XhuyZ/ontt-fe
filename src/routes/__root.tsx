		...
		<div className="min-h-screen bg-amber-50/40">
			<header className="sticky top-0 z-30 border-b border-amber-200/80 bg-amber-50/95 backdrop-blur">
				<div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3">
					<div className="flex items-center gap-3">
						<img src="/Logo.png" alt="Logo Op nhua Tuan Kiet" className="h-9 w-9 object-contain md:h-10 md:w-10" />
						<h1 className="text-sm font-semibold tracking-wide text-amber-950 md:text-base">Ốp Nhựa Tuấn Kiệt</h1>
					</div>
					...
					<nav ref={navRef} className="hidden items-center gap-5 md:flex">
						<Link to="/" className={navLinkClass} activeProps={{ className: `${navLinkClass} ${navLinkActiveClass}` }} onClick={closeAll}>
							Trang chủ
						</Link>
						...
						<div className="relative">
							...
							<div className={`absolute left-0 top-full mt-2 min-w-48 origin-top rounded-lg border border-amber-200 bg-white p-2 shadow-md transition-all duration-200 ${isProductMenuOpen ? 'pointer-events-auto translate-y-0 scale-100 opacity-100' : 'pointer-events-none -translate-y-1 scale-95 opacity-0'}`}>
								...
								{productCategories.map((name) => (
									<Link key={name} to="/san-pham" search={{ categoryId: CATEGORY_MAP[name], categoryName: name }} onClick={closeAll} className="mb-1 block rounded-md border border-amber-200 bg-white px-3 py-2 text-sm text-amber-900 last:mb-0" activeProps={{ className: 'mb-1 block rounded-md border border-amber-700 bg-amber-100 px-3 py-2 text-sm font-medium text-amber-700 last:mb-0' }}>
										{name}
									</Link>
								))}
							</div>
						...
						<Link to="/ve-chung-toi" className={navLinkClass} activeProps={{ className: `${navLinkClass} ${navLinkActiveClass}` }} onClick={closeAll}>
							Về chúng tôi
						</Link>
					</nav>
				</div>
			...
			<div className="fixed bottom-4 right-3 z-20 flex flex-col gap-2.5 md:bottom-5 md:right-4 md:gap-3">
				...
				<a
					...
					className="group relative flex h-12 w-12 items-center justify-center rounded-full border border-white/80 bg-white shadow-[0_10px_25px_rgba(0,0,0,0.2)] transition duration-300 hover:-translate-y-1 hover:scale-105 md:h-14 md:w-14"
				>
					<span className="absolute h-12 w-12 rounded-full bg-blue-400/20 animate-pulse [animation-delay:300ms] md:h-14 md:w-14" />
					<img src="/facebook-logo.png" alt="Facebook" className="relative z-10 h-7 w-7 object-contain md:h-8 md:w-8" />
				</a>
			</div>
		</div>
	)
